...
cond_t CondCreate(lock_t lock) {
  ...
  // if (AQueueInit (&conds[cond].next_q) != QUEUE_SUCCESS) {
  //   printf("FATAL ERROR: could not initialize CV next queue!\n");
  //   exitsim();
  // }
...
int CondHandleWait(cond_t c) {
  ...
  lk = conds[c].lock;
  if (lk < 0 || lk >= MAX_LOCKS || !locks[lk].inuse) {
    RestoreIntrs(intrval);
    return SYNC_FAIL;
  }
  if(locks[conds[c].lock].pid != GetCurrentPid()){
    RestoreIntrs(intrval);
    return SYNC_FAIL;
  }
  ...
  if (AQueueInsertLast (&conds[c].wait_q, l) != QUEUE_SUCCESS) {
    printf("FATAL ERROR: could not insert new link into CV waiting queue in CondHandleWait!\n");
    exitsim();
  }

  LockRelease(&locks[lk]);
  ProcessSleep();
...
int CondHandleSignal(cond_t c) {
  ...
  if(AQueueEmpty(&conds[c].wait_q)) {
    RestoreIntrs(intrval);
    return SYNC_SUCCESS;
  }

  l = AQueueFirst(&conds[c].wait_q);
  waiter = (PCB *)AQueueObject(l);
  if (AQueueRemove(&l) != QUEUE_SUCCESS) { 
    printf("FATAL ERROR: could not remove link from CV wait queue in CondHandleSignal!\n");
    exitsim();
  }

  if ((l = AQueueAllocLink ((void *)currentPCB)) == NULL) {
    printf("FATAL ERROR: could not allocate link for lock queue in CondHandleSignal!\n");
    exitsim();
  }
  if (AQueueInsertLast(&locks[lk].waiting, l) != QUEUE_SUCCESS) {
    printf("FATAL ERROR: could not insert new link into lock queue in CondHandleSignal!\n");
    exitsim();
  }

  locks[lk].pid = GetPidFromAddress(waiter);
